...
class MBL_3DOFDDInputDisplacementMM_2DPolarFeatureOM(PolarMapFeature, FEKFMBL,
    ...
    * :class:`EKF_3DOFDifferentialDriveInputDisplacement`: EKF for 3 DOF Differential Drive Mobile Robot with input displacement motion model.
    ...
    def __init__(self, *args):
        self.Feature = globals()["PolarFeature"]
...
if __name__ == '__main__':
    M = [c2p(CartesianFeature(np.array([[-40, 5]]).T)),
             ...
             c2p(CartesianFeature(np.array([[-5, 25]]).T)),
             c2p(CartesianFeature(np.array([[-3, 50]]).T)),
             c2p(CartesianFeature(np.array([[-20, 3]]).T)),
             c2p(CartesianFeature(np.array([[40, -40]]).T))]  # feature map. Position of 2 point features in the world frame.
